...
* 구별 평균 아파트 매매가는 생각보다 변동성이 두드러졌고, 특히 극단적인 변화 양상을 보였던 관악, 동작, 강동, 강남, 서초, 송파, 양천은 증감률이 +- 20%를 넘어갔다. 때문에 해당 지역을 중심으로 극단적인 가격 변화를 일으킨 요인에 대해 알아보고자 텍스트 마이닝을 실시해보기로 했다.
...
```{r}
Keyword <- "아파트+정책"
Date <- as.Date(as.character(20200501), format ="%Y%m%d")
...
start <- as.Date(as.character(20200501), format ="%Y%m%d")
end <- as.Date(as.character(20210531), format ="%Y%m%d")

Date <- start:end
Date <- as.Date(Date, origin="1970-01-01")

page <- seq(from=1, to=41, by=10)
page <- seq(from=1, by=10, length.out = 5)

list <- c()
for(date_i in Date){
  for(page_i in page){
    dt <- format(as.Date(date_i, origin="1970-01-01"), "%Y.%m.%d")
    naver <- paste0(url_1, Keyword, url_2, Date, url_3, Date, url_4, page)
    list <- c(list, naver)
  }
}
...
url <- "https://search.naver.com/search.naver?&where=news&query=아파트+정책&pd=3&ds=2020-05-01&de=2020-05-01&start=1"

html <- read_html(url[1])
temp <- unique(html_nodes(html, '#sp_nws1')%>%
                 html_nodes(css='news_wrap api_ani_send')%>%
                 html_nodes(css='news_area')%>%
                 html_nodes(css='news_info')%>%
                 html_nodes(css="api_save_group _keep_wrap")%>%
                 html_nodes(css="info_group")%>%
                 html_nodes(css="info press")%>%
                 html_nodes('a')%>%
                 html_attr('href'))
head(temp,5)
```
