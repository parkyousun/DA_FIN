...
```
```{r}
base <- "https://search.naver.com/search.naver?&where=news&query="
query <- "아파트+정책"
base_2 <- "&pd=3&ds="
base_3 <- "&de="
base_4 <- "&start="
...
end <- as.Date(as.character(20210501), format="%Y%m%d")
date <- start:end
...
for (date_i in date){
  for(page_i in page){
    ...
    urls <- paste0(base,query,base_2,date,base_3,date,base_4,page)
    urls_list <- c(urls_list, urls)
  }
}
...
#sp_nws1 > div > div > div.news_info > div.info_group > a:nth-child(3)
news
