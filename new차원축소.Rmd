...
# 기본모델
og.gu<- gu[,c(-1,-2,-12)]
og.model<- lm(평당가격 ~ ., data = og.gu)

# 기본 회귀모델의test, train set
train.og<- gu[1:16, c(-1,-2,-12)]
test.og<-gu[17:24, c(-1,-2,-12)]
...
```
주성분 분석 결과에 따른 모델을 만들어 평균 제곱근 오차를 계산한 결과 120.7944가 나온다. 기본 회귀모델의 결과인 128.9325보다 8 정도 줄어들었다.  
기본 회귀모델의 독립변수(전철역의 수, 유흥업소 수, 보건기관 수, 도서관 방문자 수, 여권발급)
```